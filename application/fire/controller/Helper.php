...
//        header('Access-Control-Allow-Credentials:true');
        header('Access-Control-Allow-Origin:http://www.xwpeng.top');
        header('Access-Control-Allow-Credentials:true');
...
//            if (!empty($val)) $xml .= "<" . $key . ">" . "<![CDATA[" . $val . "]]>" . "</" . $key . "> ";
    ...
    static function auth(array $pids = null)
    {
        ...
        if (is_array($auth)) {
            ...
            if ($distance > 6 * 60 * 60) return Errors::AUTH_EXPIRED;
            //查找权限返回,根据uid查权限
            if (empty($pids)) return [true , $data];
            $dbPids = UserDb::queryPids($data['s_uid']);
            if (!$dbPids[0]) return Errors::AUTH_PREMISSION_EMPTY;
            $a = count($pids);
            foreach ($dbPids[1] as $arr) {
                if (in_array($arr['pid'], $pids)) $a--;
            }
            return $a === 0 ? [true , $data] : Errors::AUTH_PREMISSION_REJECTED;
        } else return [false ,$auth];
    }
    ...
    {
        return (is_array(self::auth([1])) || $suid === $adder) ? [true] :Errors::LIMITED_AUTHORITY;
    }
